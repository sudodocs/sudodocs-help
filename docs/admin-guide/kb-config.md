...
Configure how the SudoDocs AI understands your product terminology, tone, and structure. See [CLI Tasks → Knowledge Base](../cli-guide/kb-config.md) for the command-line equivalent of everything on this page.
...
List your product areas (one per line) to help the AI sort and place content - used by both [Compose Release Notes](../user-guide/release-composer.md) (to bucket items into sections) and [Author a Feature](../user-guide/feature-author.md)'s Information Architect step (as a hint when deciding where new content belongs).
...
> Looking for Deployment Tags or CSV Column Mappings? Those moved to [Compose Release Notes](../user-guide/release-composer.md#release-notes-settings) - they're specific to Bulk Release Notes generation, not a Knowledge Base concern shared across features the way Product Categories is.
