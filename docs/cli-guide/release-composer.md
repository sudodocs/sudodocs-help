...
CLI equivalent of the settings behind [User Tasks → Compose Release Notes](../user-guide/release-composer.md#release-notes-settings): Deployment Tags and CSV Column Mappings. Not admin-only - available to any doc-team member, matching the settings panel on the Release Composer page itself.
...
Prints deployment tags and CSV column mappings. Empty fields mean the built-in defaults are in effect - see [Release Notes Settings](../user-guide/release-composer.md#release-notes-settings) for what those defaults are.
