...
CLI equivalents of two dashboard pages: [Admin Tasks → Configure Docflows Webhook](../../saas-guide/admin/doc-drift.md) (admin-only setup) and [User Tasks → Manage Docflows](../../saas-guide/user/doc-drift.md) (triggering a suggestion). Follow the two linked pages above for the concepts (what a webhook does, how a suggestion gets reviewed) - this page covers the equivalent commands.
...
Prints the same **Payload URL** and **Secret** the dashboard's Webhook dialog shows (admin-only) - paste these into GitHub's or Jira's webhook settings the same way as [Setting up the Webhook in GitHub/Jira](../../saas-guide/admin/doc-drift.md#step-2-set-up-the-webhook-in-github).
...
Admin-only. CLI equivalent of [Configure Screenshot Settings for Docflows](../../saas-guide/admin/doc-drift.md#configure-screenshot-settings-for-docflows).
...
The generated suggestion goes through the same Information Architect / Technical Writer / Editor pipeline as a webhook-triggered one, and lands in the same **Docflows** inbox for [triage](../../saas-guide/user/doc-drift.md#triage-suggestions) - the CLI only replaces the trigger, not the review or push steps, which stay browser-first.
