...
CLI equivalent of [Admin Tasks → Provider](../../saas-guide/admin/llm-provider.md). Admin-only, and Enterprise-only, same as the Provider tab.
...
`--provider-type` is one of `gemini`, `openai`, `claude`, `deepseek`, `custom_openai_compatible` (add `--base-url` for the last one) - see [Text Generation](../../saas-guide/admin/llm-provider.md#text-generation).
