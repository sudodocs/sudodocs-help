...
CLI equivalents of [Admin Tasks → Knowledge Base](../admin-guide/kb-config.md). All commands are admin-only, same as the Knowledge Base tab.

> Looking for Doc Team Roles, Deployment Tags, or CSV Column Mappings? Those are separate pages: [Doc Team Roles](doc-team-roles.md) and [Compose Release Notes](release-composer.md) - see [Configure the Knowledge Base](../admin-guide/kb-config.md#product-categories) for why they live where they do.
...
`--framework` is one of `google`, `microsoft`, `apple`, `ibm` - see [Set Base Frameworks](../admin-guide/kb-config.md#set-base-frameworks) for what each one means.
...
Same as clicking **Run Auto-Learn** - scans your connected Documentation/Web repositories to extract tone and terminology, and merges the result into your existing configuration (see [Auto-Learn Tone](../admin-guide/kb-config.md#auto-learn-tone)). Blocks until the job completes.
